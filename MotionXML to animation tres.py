...
for motionXML in motionXMLs:
    ...
    #print(animationName)
    animNameParts = pyUtils.SplitAnimationName(motionXML.parent.stem)
    #print(animNameParts)
    variantTag = "" if len(animNameParts) < 3 else animNameParts[2]
    objectName = animNameParts[0]
    animationName += ' - ' + objectName
    animVariantName = ""
    if len(variantTag) > 0:
        animationName +=  " ({:s})".format(variantTag)
    #print(animationName)
    fileHeaderString = ANIM_HEADER.format(animationName, float(xml.Motion["duration"]) / frameRate, baseValues["frameTime"])
    ...
    tresFile = Path(xmlFilePath.parent).joinpath(animationName + ".tres")
    #print(str(tresFile))
