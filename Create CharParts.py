...
texFolder = Path(TEX_FOLDER_PATH)
...
[ext_resource path="res://CharacterPart.gd" type="Script" id=1]
{:s}
'''
EXT_RES_STR = '''[ext_resource path="res://{:s}" type="{:s}" id={:d}]\n'''
ANIM_PLAYER_EXTRES_STR = '''"anims/{:s}" = ExtResource( {:d} )\n'''
BASE_NODE_STR = '''[node name="{:s}" type="Node2D" index="0"]
script = ExtResource( 1 )
initLoadTextures = PoolStringArray( {:s} )
currentTexture = -1

[node name="SpriteHolder" type="Node2D" parent="." index="0"]
...
chPartsFolder = Path(CHARPARTS_FOLDER_PATH)
for charPart in cleanPartsList:
    ...
    for folderName in currentPartFolders:
        ...
        for animRes in animResList:
            variantSplitList = re.split("(-|_)", str(animRes.parent.stem), maxsplit=1)
            variantTag = "" if len(variantSplitList) < 3 else variantSplitList[2]
            #print(variantTag)
            animResourceString += EXT_RES_STR.format(str(animRes.resolve().relative_to(PROJECT_ROOT_PATH)), "Animation", resCounter)
            animResName = str(animRes.stem)
            if len(variantTag) > 0:
                animResName += " ({:s})".format(variantTag)
    ...
    for texFolderName in currentPartTexFolders:
        ...
        for texTscn in texTscns:
            if len(texturePathStrings) > 0:
                texturePathStrings += ", "
            texturePathStrings += '\"' + str(texTscn.resolve().relative_to(PROJECT_ROOT_PATH)) + '\"'
